...
class FragmentVectorizer:
    ...
    def train_FastText_model(self):
        string = self.fragments
        model = FastText(self.fragments, min_count=1, vector_size=self.vector_length,
    ...
    def train_Doc2vec_model(self):
        tagged_documents = []
        for i, doc in enumerate(self.fragments):
          tagged_doc = TaggedDocument(words=doc, tags=[i])
          tagged_documents.append(tagged_doc)   
        model = Doc2Vec(tagged_documents, min_count=1, vector_size=self.vector_length)
        self.embeddings = model.wv
        del model
        del self.fragments